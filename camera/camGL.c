...
static int camGL_processCameraFrame(CamGL *camGL, void *frameBuffer)
{
	...
	for (i = 0; i < MAX_SIMUL_FRAMES; i++)
	{
		...
		{ // Found unused frame - buffer has yet to be cached and associated with an image
			...
			{
				frameInt->eglImageY = eglCreateImageKHR(camGL->eglSetup.display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, (EGLClientBuffer)frameBuffer, createAttributes);
				...
				{
					frameInt->eglImageU = eglCreateImageKHR(camGL->eglSetup.display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, (EGLClientBuffer)frameBuffer, createAttributes);
					CHECK_EVAL(frameInt->eglImageU != EGL_NO_IMAGE_KHR, "Failed to convert frame buffer to U EGL image!", errorKHRU);
					frameInt->eglImageV = eglCreateImageKHR(camGL->eglSetup.display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, (EGLClientBuffer)frameBuffer, createAttributes);
